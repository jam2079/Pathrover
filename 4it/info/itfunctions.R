...
mca <- function(C, i, j, atom) {
    ...
    result <- svd(C[Li, Lj])
    calculate.r <- function(result, C, i, j, d){
        Li <- NULL
        for (m in i) {
            Li <- c(Li, get.list(m, atom))
        }
        ...
        result$u[,d]%*%C[Li, Lj]%*%result$v[,d]/(sqrt(result$u[,d]%*%C[Li, Li]%*%result$u[,d])*sqrt(result$v[,d]%*%C[Lj, Lj]%*%result$v[,d]))
    }
    list(result, sapply(1:length(result$d), calculate.r, result=result, C=C, i=i, j=j), sqrt(mean(result$d)))
}

total.intercorrelation.fixed <- function(C, i, j, atom) {
    union_ij <- union(i, j)
    ...
    sum(0.5*log(2*pi*exp(1)*diag(C[Lij, Lij]))) - entropy(C, union_ij, atom) - conditional.total.correlation.full.resolution(C, setdiff_i, j, atom) - conditional.total.correlation.full.resolution(C, setdiff_j, i, atom)
}